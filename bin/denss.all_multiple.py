...
def multi_denss(niter, **kwargs):
    try:
        ...
        logging.info('Data filename: %s', superargs.filemultiple)
        logging.info('Output prefix: %s', kwargs['output'])
        logging.info('Mode: %s', superargs.mode)
        result = saxs.denss_multiple(**kwargs)
        logging.info('END')
        return result
        time.sleep(1)
...
if __name__ == "__main__":
    ...
    basename, ext = os.path.splitext(superargs.filemultiple[0])
    ...
    superlogger.info('Data filename: %s', superargs.filemultiple)
    superlogger.info('Enantiomer selection: %r', superargs.enan)

    denss_inputs = {'scattering_data':scattering_data}
    ...
    all_Imean = []
    for k in range(len(superargs.filemultiple)):
        all_Imean.append([denss_outputs[i][4][k] for i in np.arange(superargs.nmaps)])
    header = ['q','I','error']
    fit = np.zeros(( len(qbinsc),superargs.nmaps+3 ))

    ### Create a for loop for each file
    temp_output = output
    for k in range(len(superargs.filemultiple)):
        output = superargs.filemultiple[k].split(".")[0] #should get just the file name
        fit[:len(qdata[k]),0] = qdata[k]
        fit[:len(Idata[k]),1] = Idata[k]
        fit[:len(sigqdata[k]),2] = sigqdata[k]

        for map in range(superargs.nmaps):
            fit[:len(all_Imean[k][0]),map+3] = all_Imean[k][map][-1]
        ...
        all_chis = np.array([denss_outputs[i][5][k] for i in np.arange(superargs.nmaps)])
        all_rg = np.array([denss_outputs[i][6][k] for i in np.arange(superargs.nmaps)])
        all_supportV = np.array([denss_outputs[i][7][k] for i in np.arange(superargs.nmaps)])
    ...
    output = temp_output #restore output to whatever it was before
